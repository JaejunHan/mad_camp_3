...
import com.example.reappearance01.databinding.ActivityMapBinding
...
import com.example.reappearance01.databinding.ActivitySelectfrommapBinding
import kotlinx.coroutines.*
import kotlin.coroutines.CoroutineContext

class SelectFromMapActivity : AppCompatActivity(), OnMapReadyCallback, CoroutineScope {
    ...
    private lateinit var binding: ActivitySelectfrommapBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivitySelectfrommapBinding.inflate(layoutInflater)
        ...
        */
        ...
        binding.textView.text = locFinalName
    }
    ...
    override fun onMapReady(map: GoogleMap) {
        ...
        this.map.setOnMapClickListener {
            ...
            var fornowLocationLatLng = it
            var fornow = LocationLatLngEntity(it.latitude.toFloat(), it.longitude.toFloat())
            launch(coroutineContext) {
                try {
                                ...
                                body?.let {
                                    currentSelectMarker = setupMarker(
                                        SearchResultEntity(
                                            ...
                                            name = "선택된 위치",
                                            locationLatLng = fornow
                                        )
                                    )
                                    // 마커 보여주기
                                    currentSelectMarker?.showInfoWindow()
                                    locFinalLatLng = fornow
                                    locFinalName = it.addressInfo.fullAddress ?: "주소 정보 없음"
                                    Log.d(TAG, "옛다 locfinalname 봐라"+locFinalName)
                                    Log.d(TAG, "옛다 finallatlng latitude 봐라"+locFinalLatLng.latitude.toString())
                                    binding.textView.text = locFinalName
                                }
                            }
                        }
                    ...
                    e.printStackTrace()
                    Toast.makeText(this@SelectFromMapActivity, "검색하는 과정에서 에러가 발생했습니다.", Toast.LENGTH_SHORT).show()
                } finally {
